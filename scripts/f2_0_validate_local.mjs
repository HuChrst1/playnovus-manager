...
async function run() {
  ...
  // S0 - creation lot gratuit non-LOT_0 + cout unitaire explicite a 0
  {
    const suffix = `${Date.now()}_${Math.floor(Math.random() * 100000)}`;
    const pieceRef = `F2P_ZERO_${suffix}`;
    let lotId = null;

    const negativeRejected = await actions
      .createLotFromDialog({
        purchaseDate: "2026-02-18",
        label: `F2 Validate zero negative ${suffix}`,
        supplier: "F2 Validation",
        totalCost: -1,
        notes: "Generated by scripts/f2_0_validate_local.mjs (zero-cost negative guard)",
      })
      .then(() => false)
      .catch((error) =>
        String(error instanceof Error ? error.message : error).includes("positif")
      );
    assert(negativeRejected, "S0 echec: un cout total negatif devrait etre refuse.");

    try {
      const createResult = await actions.createLotFromDialog({
        purchaseDate: "2026-02-18",
        label: `F2 Validate zero cost ${suffix}`,
        supplier: "F2 Validation",
        totalCost: 0,
        notes: "Generated by scripts/f2_0_validate_local.mjs (zero-cost lot)",
      });

      assert(
        createResult?.success === true && Number.isFinite(Number(createResult.lotId)),
        `S0 echec creation lot gratuit: ${JSON.stringify(createResult)}`
      );
      lotId = Number(createResult.lotId);

      const createdLot = await loadLotForUpdate(admin, lotId);
      assert(createdLot.status === "draft", `S0 echec: statut draft attendu, recu ${createdLot.status}`);
      assert(Number(createdLot.total_cost ?? -1) === 0, `S0 echec: total_cost attendu 0, recu ${createdLot.total_cost}`);

      const addResult = await actions.addPieceToLot(lotId, {
        pieceRef,
        quantity: 4,
      });
      assert(addResult?.success === true, `S0 echec ajout piece lot gratuit: ${JSON.stringify(addResult)}`);

      const lotAfterAdd = await querySingle(admin, "lots", "total_pieces, total_cost", [
        ["eq", "id", lotId],
      ]);
      assert(Number(lotAfterAdd?.total_pieces ?? -1) === 4, `S0 echec: total_pieces attendu 4, recu ${lotAfterAdd?.total_pieces}`);
      assert(Number(lotAfterAdd?.total_cost ?? -1) === 0, `S0 echec: total_cost apres ajout attendu 0, recu ${lotAfterAdd?.total_cost}`);

      const invLine = await getInventoryLineForLotPiece(admin, lotId, pieceRef);
      assert(invLine, "S0 echec: ligne inventory absente apres ajout lot gratuit.");
      assert(Number(invLine.quantity ?? 0) === 4, `S0 echec: quantity inventory attendue 4, recue ${invLine.quantity}.`);
      assert(Number(invLine.unit_cost ?? -1) === 0, `S0 echec: unit_cost inventory attendu 0, recu ${invLine.unit_cost}.`);

      const confirmResult = await actions.updateLotFromDialog(
        lotId,
        toUpdateArgs(await loadLotForUpdate(admin, lotId), "confirmed")
      );
      assert(confirmResult?.success === true, `S0 echec confirmation lot gratuit: ${JSON.stringify(confirmResult)}`);

      const confirmedLot = await loadLotForUpdate(admin, lotId);
      assert(
        confirmedLot.status === "confirmed",
        `S0 echec: statut confirmed attendu, recu ${confirmedLot.status}`
      );

      const purchaseMovement = await getPurchaseMovementForLotPiece(admin, lotId, pieceRef);
      assert(purchaseMovement, "S0 echec: mouvement PURCHASE/IN absent apres confirmation lot gratuit.");
      assert(
        Number(purchaseMovement.quantity ?? 0) === 4 &&
          Number(purchaseMovement.unit_cost ?? -1) === 0,
        `S0 echec: PURCHASE attendu q=4, unit_cost=0 (recu q=${purchaseMovement.quantity}, uc=${purchaseMovement.unit_cost}).`
      );
    } finally {
      if (lotId !== null) {
        await cleanupLotHard(admin, lotId);
      }
    }

    record("S0 lot gratuit accepte + cout unitaire FIFO 0");
  }
